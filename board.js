...
Board.prototype.getAdjacentFriends = function(i, j, iInc, jInc) {
	var adjFriends = 0;
	while(true) {
		i = i + iInc;
		j = j + jInc;
		...
		if(this.board[i][j] == this.current_color) {
			adjFriends++;
		}
		else {
			break;
...
Board.prototype.checkWin = function() {
	var i = this.curI;
	var j = this.curJ;
	// Check the back slash direction.
	var dirBS = this.getAdjacentFriends(i, j, 1, 1) + this.getAdjacentFriends(i, j, -1, -1);
	var dirFS = this.getAdjacentFriends(i, j, -1, 1) + this.getAdjacentFriends(i, j, 1, -1);
	var dirHR = this.getAdjacentFriends(i, j, 0, 1) + this.getAdjacentFriends(i, j, 0, -1);
	var dirVT = this.getAdjacentFriends(i, j, 1, 0) + this.getAdjacentFriends(i, j, -1, 0);
	if(dirBS > 3 || dirFS > 3 || dirHR > 3 || dirVT > 3) {
		this.winnerColor = this.current_color;
		//alert("Color " + this.winnerColor + " won!");
	}
	// Debug.
	//console.log(i + " " + j + " " + dirBS + " " + dirFS + " " + dirHR + " " + dirVT);
}
